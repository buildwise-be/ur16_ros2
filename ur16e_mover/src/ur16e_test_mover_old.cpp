...
class MoveUr16e : public rclcpp::Node, public std::enable_shared_from_this<MoveUr16e> {
public:
  MoveUr16e()
  : Node("move_ur16e") {
    // Create service
...
private:
  ...
  void handle_service(
    ...
    auto trajectories = plan_move(request->joints, request->target_pose);
  ...
  std::vector<moveit_msgs::msg::RobotTrajectory> plan_move(
    ...
    bool success = (planner_->plan(plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
    if (!success) {
